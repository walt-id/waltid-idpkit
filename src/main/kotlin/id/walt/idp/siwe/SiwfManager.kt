...
object SiwfManager{
  ...
  fun verifySignature(session: OIDCSession, message: String, publicKey: String, signature: String):Boolean{
    val nonce= SiwfManager.getNonce(message)
    ...
    return runBlocking {
      val result = SiwfManager.client.get("${IDPConfig.config.jsProjectExternalUrl}/flow/signature/verification?publicKey=${publicKey}&signature=${signature}&message=${URLEncoder.encode(message, StandardCharsets.UTF_8)}") {
      }.body<Boolean>()
