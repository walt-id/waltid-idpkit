...
@Serializable
data class SignatureVerificationResultNear(
    val result: Boolean
)
...
object SiwnManager {
    ...
    fun verifySignature(session: OIDCSession, message: String, publicKey: String, signature: String): Boolean{
        ...
        return runBlocking {
            val result = client.get("${IDPConfig.config.jsProjectExternalUrl}/near/signature/verification?publicKey=${publicKey}&signature=${signature}&message=${URLEncoder.encode(message, StandardCharsets.UTF_8)}") {
            }.body<Boolean>()
